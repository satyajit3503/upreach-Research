...
const ReviewerCamera = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (validation()) {
            if (data.status < 2) {
                axios.post('http://localhost:5000/cameraapi/form', { application_no, comments, marks, reviewer_crn, status })
                    .then((res) => {
                        if (res.data === "success") {
        ...
        <>
            ...
            <div className="container d-flex my-5 p-0" style={{ height: "60vh" }} >
                <div className="container  w-50 d-flex align-items-center justify-content-evenly flex-column rounded p-2" style={{ backgroundColor: "#caf0f8" }}>
                    ...
                    <h6>Click below link for start camera screening meeting</h6>
